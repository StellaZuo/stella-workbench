...
(function () {
  ...
  // ===== 云端合并引擎（防止本地整行覆盖，抹掉里里推来的数据） =====
  const SEEN_MAX = 400;
  function ensureSeen(d) { if (!Array.isArray(d._seen)) d._seen = []; return d._seen; }
  function markSeen(id) {
    const s = ensureSeen(data); const k = String(id);
    if (!s.includes(k)) s.unshift(k);
    if (s.length > SEEN_MAX) s.length = SEEN_MAX;
  }
  window.__markSeen = markSeen;

  const DATE_DICTS = ['weights', 'diets', 'fitness'];
  const SEED_ARRAYS = ['biz_inbox', '_inbox'];
  const FEED_KEYS = ['news', 'financeNews', 'macroNews', 'bookRecs'];

  function idOf(it, i) {
    if (it && it.id != null) return String(it.id);
    try { return 'h' + JSON.stringify(it); } catch (e) { return 'i' + i; }
  }
  function unionById(a, b, seen, preferA) {
    const A = Array.isArray(a) ? a : [], B = Array.isArray(b) ? b : [];
    const map = new Map();
    (preferA ? B.concat(A) : A.concat(B)).forEach((it, i) => {
      const k = idOf(it, i);
  ...
  // mode='push' 本地优先（保住刚编辑的）；mode='pull' 投递型数据取远端
  function mergeStates(remote, local, mode) {
    remote = remote || {}; local = local || {};
    const seen = new Set([].concat(local._seen || [], remote._seen || []).map(String));
    const out = Object.assign({}, remote, local); // 远端打底 + 本地覆盖 → 远端独有键不丢

    DATE_DICTS.forEach(k => {
      const r = remote[k] || {}, l = local[k] || {};
      const m = Object.assign({}, r);
      Object.keys(l).forEach(day => {
        const lv = l[day], rv = r[day];
        if (Array.isArray(lv)) m[day] = (lv.length ? lv : (Array.isArray(rv) ? rv : lv));
        else if (lv && typeof lv === 'object') m[day] = Object.assign({}, rv || {}, lv);
        else m[day] = lv;
      });
      out[k] = m;
    });

    SEED_ARRAYS.forEach(k => {
      out[k] = unionById(local[k], remote[k], seen, true)
        .sort((x, y) => (Number(y && y.id) || 0) - (Number(x && x.id) || 0));
    });

    const rAs = Date.parse(remote.newsAsOf || 0) || 0;
    const lAs = Date.parse(local.newsAsOf || 0) || 0;
    const useRemoteFeed = mode === 'pull' ? (rAs >= lAs) : (rAs > lAs);
    FEED_KEYS.forEach(k => {
      const rv = remote[k], lv = local[k];
      const rEmpty = !rv || (Array.isArray(rv) && !rv.length);
      const lEmpty = !lv || (Array.isArray(lv) && !lv.length);
      if (lEmpty && !rEmpty) out[k] = rv;
      else if (useRemoteFeed && !rEmpty) out[k] = rv;
      else out[k] = lEmpty ? rv : lv;
    });
    if (useRemoteFeed && remote.newsAsOf) out.newsAsOf = remote.newsAsOf;

    Object.keys(out).forEach(k => {
      if (DATE_DICTS.indexOf(k) >= 0 || SEED_ARRAYS.indexOf(k) >= 0 || FEED_KEYS.indexOf(k) >= 0 || k === '_seen') return;
      const lv = local[k], rv = remote[k];
      if (Array.isArray(rv) && (!Array.isArray(lv) || !lv.length)) out[k] = rv;
    });

    out._seen = Array.from(seen).slice(0, SEEN_MAX);
    return out;
  }
  ...
  async function doPush() {
    ...
    try {
      // 先拉远端做合并再上传，绝不整行覆盖（否则会抹掉里里刚推来的种子/数据）
      let merged = data, needReload = false;
      try {
        const row = await sbGetRow();
        if (row && row.payload) {
          merged = mergeStates(row.payload, data, 'push');
          const a = JSON.stringify({ b: data.biz_inbox || [], i: data._inbox || [] });
          const b = JSON.stringify({ b: merged.biz_inbox || [], i: merged._inbox || [] });
          if (a !== b) { localStorage.setItem(DB_KEY, JSON.stringify(merged)); needReload = true; }
        }
      } catch (e) { if (e.message === 'TABLE_MISSING') throw e; }
      await sbUpsertRow(merged, ts);
      if (needReload) setTimeout(() => location.reload(), 400);
      const m = loadMeta();
      m.syncedAt = ts;
      m.localModified = ts;
      saveMeta(m);
    ...
    try {
      ...
      if (row) {
        ...
        if (remoteTs > syncedAt) {
          // 远端有更新 → 合并（不再整行覆盖本地）
          const merged = mergeStates(row.payload || {}, data, localMod > syncedAt ? 'push' : 'pull');
  ...
  function renderInbox() {
    ...
    box.querySelectorAll('.del').forEach(d => d.onclick = () => {
      const i = inbox.findIndex(x => x.id == d.dataset.id);
      if (i >= 0) { markSeen(inbox[i].id); inbox.splice(i, 1); }
      save(); renderInbox();
    });
  }
  ...
  async function pullUpdates() {
    ...
    try {
      ...
      const localMod = meta.localModified ? Date.parse(meta.localModified) : 0;
      const remoteTs = row.updated_at ? Date.parse(row.updated_at) : 0;
      const before = JSON.stringify({ b: data.biz_inbox || [], i: data._inbox || [], n: data.newsAsOf || '' });
      const merged = mergeStates(row.payload || {}, data, 'pull');
      const after = JSON.stringify({ b: merged.biz_inbox || [], i: merged._inbox || [], n: merged.newsAsOf || '' });
      if (before !== after || remoteTs > syncedAt) {
        localStorage.setItem(DB_KEY, JSON.stringify(merged));
        saveMeta({ syncedAt: new Date().toISOString(), localModified: new Date().toISOString(), device: row.device });
        setStatus('online');
